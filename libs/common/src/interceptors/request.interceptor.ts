  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    response.on('finish', () => {
      ...
      if (!this.isWhiteListed(request)) {
        request.logger.log(`${text.completed} ${method} ${code} in ${delta}ms ${originalUrl} - ${userAgent} ${ip}`);
      }
    });

    if (!this.isWhiteListed(request)) {
      request.logger.log(`${text.started} ${method} ${originalUrl} - ${userAgent} ${ip}`);
      request.logger.log(`${text.payload} ${JSON.stringify(request.body)}`);
    }
